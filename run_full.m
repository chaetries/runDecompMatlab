%% Full CVAE - Cervix Tissue Analysis
clear; clc;

%% Configuration
decoder_file = 'model/full/cvae_cervix_decoder_best.onnx';
decoder_function = 'model/full/DecoderFunction';
data_file = 'sample/full/SAMMM.mat';

%% Load Data
fprintf('Loading full Mueller matrix data...\n');
data = load(data_file);
nM = data.nM; % Shape: [H, W, 16]
M11s = data.M11s; % Shape: [H, W]
[H, W, ~] = size(nM);
N = H * W;
mueller_data = reshape(nM, N, 16);
